...
> **MAX endstops — one pin per axis, no resistors:** each of the 6 switches gets its own GPIO with the internal `INPUT_PULLUP` (no external resistors), NC wiring COM→GND, so the firmware knows exactly which axis tripped: X_MAX→GPIO4, Y_MAX→GPIO18, Z_MAX→GPIO17. Closed (not at limit) = LOW; switch open or broken wire = HIGH → fault/stop (fail-safe). NC endstops idle LOW, so they can't go on strapping pins that must be HIGH at boot — but the **CAM scan trigger** is an output (UART2 TX) that idles HIGH, so it fits **GPIO5** (strapping, must be HIGH at boot) with no boot conflict. That keeps all 6 endstops per-axis **and** the hardware trigger (CAM stays trigger-based, §8). GPIO15 is an equivalent alternative; avoid GPIO12/0/2 for the trigger. The MAX switches are optional.
...
- **Homing references the MIN switches (GPIO 21/22/23).** The MAX switches (GPIO 4/18/17, one per axis) are hard end-of-travel limits: read them in the motion loop and stop/fault on trip — the per-axis pins tell you exactly which axis over-traveled.
- Define **soft limits** (software limits in mm) per axis, so you don't force the end of travel even before a MAX switch trips.
...
AT_SCAN
  └─> send trigger to CAM (GPIO5), wait for QR (timeout)
...
```
**brain → CAM (control, UART2 TX / GPIO5 → CAM GPIO14):**
```
