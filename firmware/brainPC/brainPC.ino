...
bool autoMode = false;             // R1: park at SCAN, sort each QR, return to SCAN (loop)
...
void printHelp() {
  Serial.println(F(
    ...
    "  Q1/Q0  auto-sort on a single QR (on/off)\n"
    "  R1/R0  AUTO loop: park at SCAN, sort each QR, return to SCAN (R0/S to stop)\n"
    "  V<mm/s> speed   A<mm/s2> accel   S stop   P status   ?  help\n"
...
void handleLine(String s) {
  ...
  switch (c) {
    ...
    case 'Q':
      ...
      break;
    case 'R':                       // AUTO loop: park at SCAN, sort each QR, return to SCAN
      if (arg.toInt() == 1) {
        if (!allHomed()) { Serial.println(F("[R] home first (HA)")); break; }
        autoMode = true; autosort = true;
        Serial.println(F("[R] AUTO ON -> going to SCAN, waiting for QR"));
        gotoPos(SCAN_POS);
      } else {
        autoMode = false;
        Serial.println(F("[R] AUTO OFF"));
      }
      break;
    case 'P':
      printStatus();
...
void handleCamLine(String line) {
  ...
  if (line.startsWith("QR:")) {
    ...
    // one box at a time: after a sort, ignore further QRs for a few seconds so the
    // stream of repeated detections doesn't loop the cycle while you hold the QR.
    static unsigned long lastSortMs = 0;
    if (lastSortMs && millis() - lastSortMs < 5000) return;
    if (!routeForCode(code.c_str())) { Serial.printf("[CAM] unknown code '%s' -> reject\n", code.c_str()); return; }
    sortCycle(code.c_str());
    if (autoMode) { gotoPos(SCAN_POS); Serial.println(F("[auto] back at SCAN, waiting for next QR")); }
    lastSortMs = millis();
  } else if (line == "NOQR") {
...
void setup() {
  ...
  delay(300);
  Serial.println(F("\n=== BRAIN-PC (QR comes from PC decoder over USB serial as 'QR:<code>') ==="));
...
void loop() {
  // USB serial: lines starting "QR:" come from the PC decoder (decode_qr.py) ->
  // route to the sort logic; everything else is a normal console command.
  static String buf;
  while (Serial.available()) {
    char ch = (char)Serial.read();
    if (ch == '\n' || ch == '\r') {
      if (buf.length()) {
        if (buf.startsWith("QR:")) handleCamLine(buf);   // QR from the PC
        else                       handleLine(buf);      // console command
        buf = "";
      }
    } else {
      buf += ch;
  ...
  // (ESP-NOW path is still here but unused in this PC-decode variant)
  if (camMsgReady) {
